...
class ResnetGenerator(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.InstanceNorm2d, use_dropout=True, n_blocks=6,
        ...
        for i in range(6):  # add ResNet blocks
            ...
            model1 += [ResnetBlock(32,kernel=5,padding_type=padding_type, norm_layer=norm_layer, use_dropout=False,
                                  use_bias=use_bias)]

        # model1 += [nn.ReflectionPad2d(2),
        #     nn.Conv2d(64, 3, kernel_size=3,dilation=2, bias=use_bias),norm_layer(ngf),
        #     nn.ReLU(True)
        # ]
        self.model1 = nn.Sequential(*model1)

        model2 = [nn.ReflectionPad2d(2),
                  nn.Conv2d(3, 64, kernel_size=3,dilation=2, bias=use_bias),norm_layer(ngf),
                  GELU()]

        for i in range(6):  # add ResNet blocks

            model2 += [ResnetBlock(64, kernel=3, padding_type=padding_type, norm_layer=norm_layer, use_dropout=False,
                                   use_bias=use_bias)]
        model2 += [nn.ReflectionPad2d(2),
            nn.Conv2d(64, 3, kernel_size=3, dilation=2, bias=use_bias), norm_layer(32),
            GELU()
        ]
        self.model2 = nn.Sequential(*model2)
        ...
        self.conv2_128 = nn.Sequential(
            nn.Conv2d(70, 64, kernel_size=1, stride=1, dilation=1,padding=0),
            # ResnetBlock1(3,kernel=1,padding_type=padding_type, norm_layer=norm_layer, use_dropout=False,
            #                       use_bias=use_bias),
            norm_layer(64),
            GELU()
        ...
        self.fc = nn.Sequential(
            ...
            nn.Linear(70, 70),
            GELU(),
            nn.Linear(70, 1),  # 可能应该是nn.Linear(channel, channel)
            GELU(),
        )
    def forward(self, input):
        """Standard forward"""
        ...
        y1 =self.conv1(input)
        y2 =self.model1(y1)
        y3 =torch.cat([y1,y2],dim=1)#64
        y3 =self.conv3_64(y3)


        # x2 = self.model1(input) #64
        x2 = self.model2(input)#3
        # x3 = torch.cat([y1,x2],dim=1)#64
        out = torch.cat([x1,y3,x2],dim=1)
        x_raw = out
        x = out
        y = torch.abs(x)
        y_abs = y
...
class PixelDiscriminator(nn.Module):
    ...
    def __init__(self, input_nc, ndf=64, norm_layer=nn.BatchNorm2d):
        ...
        self.net = [
            ...
            norm_layer(ndf * 2),
            nn.LeakyReLU(0.2, True),
            nn.Conv2d(ndf * 2, 64, kernel_size=1, stride=1, padding=0, bias=use_bias)]
