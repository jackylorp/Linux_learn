...
from models import arch_util as arch_util
...
class ResnetGenerator(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=True, n_blocks=6,
        ...
        model = [nn.ReflectionPad2d(1),
                 nn.Conv2d(input_nc, 64, kernel_size=3, padding=0, bias=use_bias),
                 norm_layer(ngf),
                 nn.ReLU(True)]
        ...
        # model += [nn.ConvTranspose2d(64, 64,
        ...
        #           norm_layer(ngf),
        #           nn.ReLU(True)]
        # model += [nn.Upsample(scale_factor=0.5),
        #           norm_layer(64),
        #           nn.ReLU(True)]
        model += [nn.ReflectionPad2d(2),
                  nn.Conv2d(64, 64, kernel_size=3, padding=0, dilation=2, bias=use_bias),
                  ...
                  nn.ReLU(True)]
        model += [nn.ReflectionPad2d(1),
            nn.Conv2d(64, output_nc, kernel_size=3, padding=0, bias=use_bias),
        ]
        ...
        for i in range(9):  # add ResNet blocks
            ...
            model2 = [ResnetBlock(64, padding_type=padding_type, norm_layer=norm_layer, use_dropout=use_dropout,
                                  use_bias=use_bias)]
        model2 += [nn.Tanh()]
...
class ResnetBlock(nn.Module):
    ...
    def build_conv_block(self, dim, padding_type, norm_layer, use_dropout, use_bias):
        ...
        conv_block += [nn.Conv2d(dim, 128, kernel_size=3, padding=p, bias=use_bias), norm_layer(256), nn.ReLU(True)]

        if use_dropout:
            conv_block += [nn.Dropout(0.5)]
        # conv_block += [nn.Conv2d(128, 128, kernel_size=3, padding=1, bias=use_bias), nn.ReLU(True)]
        conv_block += [nn.Conv2d(128, 128, kernel_size=3, padding=p, bias=use_bias), nn.ReLU(True)]
        ...
        # else:
        #      raise NotImplementedError('padding [%s] is not implemented' % padding_type)
        conv_block += [nn.Conv2d(256, 128, kernel_size=3, padding=p, bias=use_bias), nn.ReLU(True)]
        conv_block += [nn.Conv2d(128, 128, kernel_size=3, padding=p, bias=use_bias), nn.ReLU(True)]
        conv_block += [nn.Conv2d(128, dim, kernel_size=3, padding=p, bias=use_bias), norm_layer(dim)]
